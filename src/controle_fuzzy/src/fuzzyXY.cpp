...
//---------------------------
  if (ros::ok())
  {
    ...
    float tolerance_orie = 0.006, tolerance_pos = 0.005;
    float Kpos = 15, Korie = 25;
    float angulo;
    ...
    while (abs(erroorie) > tolerance_orie && dist > tolerance_pos) {
        ...
        msg.linear.x = fc.make_inference(abs(Kpos*(dist) /(60/3)));
        ROS_INFO("Dist>>%f",dist);

        erroorie = angulo-feedback.theta;
        msg.angular.z = fc.make_inference(Korie*erroorie/(60/5));
